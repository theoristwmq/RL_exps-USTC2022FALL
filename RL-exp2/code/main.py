...
class DQN():
    ...
    def learn(self):
        ...
        q_eval = self.eval_net(b_s.cuda()).gather(1, b_a.unsqueeze(1).cuda())
        q_next = self.target_net(b_ns.cuda()).detach() # detach的作用就是不反向传播去更新，因为target的更新在前面定义好了的
        q_target = b_r.unsqueeze(1).cuda() + GAMMA * q_next.max(1)[0].view(BATCH_SIZE, 1)# shape (batch, 1)
