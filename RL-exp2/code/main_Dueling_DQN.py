...
class DQN():
    ...
    def choose_action(self, state, EPISILO = 1.0):
        ...
        if np.random.random() > EPISILO:# greedy policy
            action_value, _ = self.eval_net.cuda().forward(state)
    ...
    def learn(self):
        ...
        sample_index_tensor = torch.from_numpy(sample_index).type(torch.LongTensor)
        # TODO
        batch_memory = self.memory.get(index=sample_index)
        b_s, b_a, b_r, b_ns = batch_memory
        # q_eval w.r.t the action in experience
        with torch.no_grad():
            A_, V_ = self.target_net(b_ns.cuda())
            q_next = A_ + V_ - torch.mean(A_, dim=-1, keepdim=True).cuda()
            q_target = b_r.unsqueeze(1).cuda() + GAMMA * q_next.max(1)[0].view(BATCH_SIZE, 1)
